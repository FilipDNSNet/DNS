...
insert into comsof.comsof_metadata( 	datum,	 	bundesland,	 	_epsg_code, 			destination_cluster, 	beschreibung) 
							values( 	now(), 		'Brandenburg', 	25833,					9,						'gefoerderte Ausbau Cottbus');
...
/*  ########################        Haus anschluss            ###################################################################################################################################################################   */
/*the id of the "knoten" is the id of "OUT_ACCESSSTRUCTURE"*/
...
-- added on 29.01.2021
-- ==>  The "id" of COMSOF.ABSCHLUSSPUNKTE is the same with  the "id_0" of IN_DEMANDPOINTS
insert into comsof.abschlusspunkte (id,geom,  knoten_id, knoten_uuid,typ,  einbauort , homeconnect_status, cluster)  
	select right(pnt.fid,-16)::int in_demandpnt_id , k.geom, k.id, k.uuid , 'Gf-HUEP' typ , Null einbauort, Null homeconnect_status, (select destination_cluster from comsof.comsof_metadata) as cluster
		from comsof.out_demandpoints pnt join comsof.knoten k on k.id=pnt.id;
-- The "fid" column of OUT_DEMAND_POINTS is the "id_0" of in_demand_points.  =>  IN.id_0=OUT.right(pnt.fid,-16)::int
-- The "id" of Knoten is the "id" of OUT_ACCESSSTRUCTURES, which is the same as "id" of OUT_DEMANDPOINTS.




					-- In this case we have alredy matched the IN_DEMAND_POINTS with "adressen.adressen". The column "id" contains the uuid of the adresses.
			

				insert into  comsof.adresse_abschluss (adresse_id, abschlusspunkte_id, abschlusspunkte_uuid , cluster)
					select uuid(in_dempnt.id) adr_id , ab.id abschluss_id, ab.uuid abschluss_uuid  
						, (select destination_cluster from comsof.comsof_metadata) as cluster 
					from comsof.in_demandpoints in_dempnt join comsof.abschlusspunkte ab on in_dempnt.id_0=ab.id
					where in_dempnt.id is not null;
...
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster)
	select id, geom , 'Schacht', 'Ziehschacht / Schacht an grossen Knotenpunkten', 'Grobplanung',Null , (select destination_cluster from comsof.comsof_metadata)  from comsof.out_accessstructures where type='ManHole';

/*the id of the "connection_module" is eq_id of the "OUT_ACCESSSTRUCTURE"*/
insert into comsof.connection_module(id, typ, produkt_id, knoten_id, netzebene_quelle,netzebene_ziel, eigentum_status, foerdert_status, cluster)
	select eq_id, 'Switch / Plugin', NULL, id, 'Backbone-Ebene','Haupt-Ebene', 'Eigentum', Null, (select destination_cluster from comsof.comsof_metadata) from comsof.out_accessstructures where type in ('CentralOffice','ManHole');
with sel as( select connmodule.id, knot.uuid  knot_uuid from  comsof.connection_module connmodule join comsof.knoten knot on knot.id=connmodule.knoten_id )
...
--	select id, st_linemerge(geom), cable_id, 'Haupt-Ebene',1 beundeln, Null fs_label,
--	p_cl_eq_id, Null elem_out_nr, Null anfang_typ, Null label_anf,
--	c_cl_eq_id, null elem_in_nr, Null ende_typ, Null label_ende, 
--	Null length, (select destination_cluster from comsof.comsof_metadata) clust from comsof.temp_faser;

--or
--insert into comsof.faser (id, geom, kabel_id, netzebene,buendeln_nr , faser_label ,conn_element_anfang, anf_elem_output_nr, anfang_typ,anfang_label, conn_element_ende, end_elem_input_nr ,ende_typ, ende_label,length, cluster)
--	select id, st_linemerge(geom), cable_id, 'Haupt-Ebene',1 beundeln, Null fs_label,
--	parent_id, Null elem_out_nr, Null anfang_typ, Null label_anf,
--	child_id, null elem_in_nr, Null ende_typ, Null label_ende, 
--	Null length, (select destination_cluster from comsof.comsof_metadata) clust from comsof.temp_faser;

insert into comsof.faser (id, geom, kabel_id, netzebene,buendeln_nr , faser_label ,conn_element_anfang, anf_elem_output_nr, anfang_typ,anfang_label, conn_element_ende, end_elem_input_nr ,ende_typ, ende_label,length, cluster)
	select id, st_linemerge(geom), cable_id, 'Haupt-Ebene',1 beundeln, Null fs_label,
	case when exists (select from comsof.connection_element where id=p_cl_eq_id) then p_cl_eq_id else parent_id end conn_element_anfang , Null elem_out_nr, Null anfang_typ, Null label_anf,
	case when exists (select from comsof.connection_element where id=c_cl_eq_id) then c_cl_eq_id else child_id end conn_element_ende, null elem_in_nr, Null ende_typ, Null label_ende, 
	Null length, (select destination_cluster from comsof.comsof_metadata) clust from comsof.temp_faser;
	...
	update comsof.temp_microduct set 
		knot_anf=(select case 
			when anfang_location>=ende_location Then
				(select id from comsof.knoten where st_intersects(comsof.knoten.geom, comsof.temp_microduct.point_anf) limit 1)
			else
				(select id from comsof.knoten where st_intersects(comsof.knoten.geom, comsof.temp_microduct.point_end) limit 1)
			end)
...
---- Knoten
INSERT INTO prj_cottbus.knoten (id, bez, geom, typ, subtyp, planung_status, foerdert_status, produkt_id, label_prefix, label_wert,cluster)
	select 
...
---- Abschlusspunkte
INSERT INTO prj_cottbus.abschlusspunkte (id,geom,knoten_id,typ,einbauort,homeconnect_status,cluster)
	select 
...
-- ---- adresse_abschluss
-- INSERT INTO prj_cottbus.adresse_abschluss (adresse_id, abschlusspunkte_id, cluster)
-- 	select 
-- 		adresse_id,abschlusspunkte_uuid,cluster
-- 	from comsof.adresse_abschluss;	

---- Connection_module
INSERT INTO prj_cottbus.connection_module (id,typ,produkt_id,knoten_id,netzebene_quelle,netzebene_ziel,eigentum_status,foerdert_status, planung_status,cluster)
	select 
...
---- connection_unit
INSERT INTO prj_cottbus.connection_unit(id,conn_module_id,produkt_id,letzten_datum_mod,cluster)
	select 
	uuid,conn_module_uuid,produkt_id,letzten_datum_mod,cluster
	from comsof.connection_unit;	

---- Connection_element
INSERT INTO prj_cottbus.connection_element(id,conn_unit_id,produkt_id,typ,subtyp,installation_ziele,installation_spalte,label,mehrdetail,cluster)
	select 
...
---- Kabel
INSERT INTO prj_cottbus.kabel (id,bez,geom,produkt_id, anzahl_fasern,typ,netzebene,conn_module_anfang,conn_module_ende,planung_status,foerdert_status,eigentum_status,homeconnect_status,length,cluster)
	select 
...
---- Faser
INSERT INTO prj_cottbus.faser (id , bez,geom,kabel_id,netzebene, buendeln_nr,faser_label,conn_element_anfang,anf_elem_output_nr,anfang_typ,anfang_label,conn_element_ende,end_elem_input_nr,ende_typ,ende_label,length,external_id,cluster)
	select 
...
---- Rohr
INSERT INTO prj_cottbus.rohr (id,bez,geom,typ,produkt_id, anzahl_microducts,mantel_farbe,mantel_label,planung_status,foerdert_status,netzebene,homeconnect_status,cluster)
	select 
...
---- Microduct
INSERT INTO prj_cottbus.microduct(id,bez,geom,rohr_id,microduct_nr,knoten_anfang,   conn_module_anfang, conn_module_anfang_label,knoten_ende ,conn_module_ende, conn_module_ende_label,stammt_von, bottom_agg_id, zweig, kabel_id,netzebene, bez_wert,cluster) 
	select 
...
---- Trasse
INSERT INTO prj_cottbus.trasse(id,geom,typ,trassenbauverfahren,sonst_bauverfahren,verlege_tief_m,oberflaeche,widmung,foerdert_status,planung_status,homeconnect_status,netzebene,cluster)
	select 
...
---- Schutzrohr
INSERT INTO prj_cottbus.schutzrohr (id,geom,produkt_id,foerdert_status,planung_status,aggregation_id,cluster)
	select 
		uuid,geom,produkt_id,foerdert_status,planung_status,aggregation_id,cluster
	from comsof.schutzrohr;	
	
---- rohr_schutzrohr
INSERT INTO prj_cottbus.rohr_schutzrohr (schutzrohr_id,rohr_id,cluster)
	select 
		schutzrohr_uuid,rohr_uuid,cluster
	from comsof.rohr_schutzrohr;
	
---- strecke
INSERT INTO prj_cottbus.strecke (_id,typ,cluster)
	select 
		_uuid,typ,cluster
	from comsof.strecke;	
