...
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster) select id, geom , 'Gebaeude', 'Abschlusspunkt', 'Grobplanung',null , 1  from comsof.out_accessstructures where type='Building';
...
with sel as (select ge.id knot_id, uuid knot_uuid ,fid, ge.geom from comsof.knoten ge join comsof.in_demandpoints on st_equals(ge.geom, comsof.in_demandpoints .geom)) 
	insert into comsof.abschlusspunkte (id,geom,  knoten_id, knoten_uuid,typ,  einbauort , homeconnect_status, cluster)  select fid , geom, knot_id ,knot_uuid , 'Gf-HUEP', Null, Null , 1 from sel ;
...
insert into comsof.adresse_abschluss(abschlusspunkte_id, adresse_id, cluster)  select    abschluss_id, adresse_id, 1 from comsof.temp;
/* ==>  The id of "a.abschlusspunkte" is the same with  the fid of "in_demandpoints" . because we need to match it with amtlichen daten*/
...
insert into comsof.connection_module(id, typ, produkt_id, knoten_id, netzebene_quelle,netzebene_ziel, eigentum_status, foerdert_status, cluster) 
	select eq_id, 'Switch / Plugin', NULL, id, 'Verteiler-Ebene','In-House-Ebene', 'Eigentum', 'Eigenausbau',1 from comsof.out_accessstructures where type = 'Building';
...
insert into comsof.connection_unit (id, conn_module_id,	produkt_id, letzten_datum_mod, cluster)
	select  eq_id , enc_eqp_id ,Null , Null, 1 from comsof.out_closures where layer='Demand';
...
insert into  comsof.connection_element( id,	conn_unit_id,	produkt_id,	typ, subtyp, installation_ziele,	installation_spalte , label,	 mehrdetail, cluster )
	select eq_id, eq_id, null, 'Passive', 'Switch / Plugin', Null, Null, Null, null,1 from comsof.out_closures where layer='Demand'; /*only for Haus anschluss*/	
...
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster )
	select id, geom , 'Gebaeude', 'POP', 'Grobplanung',Null , 1  from comsof.out_accessstructures where type='CentralOffice';

/*the id of the "connection_module" is eq_id of the "OUT_ACCESSSTRUCTURE"*/
insert into comsof.connection_module(id, typ, produkt_id, knoten_id, netzebene_quelle,netzebene_ziel, eigentum_status, foerdert_status, cluster)
	select eq_id, 'Switch / Plugin', NULL, id, 'Backbone-Ebene','Haupt-Ebene', 'Eigentum', Null,1 from comsof.out_accessstructures where type = 'CentralOffice';
with sel as( select connmodule.id, knot.uuid  knot_uuid from  comsof.connection_module connmodule join comsof.knoten knot on knot.id=connmodule.knoten_id )
...
/*The id of the "connection_unit" is the eq_id of the "out_closure" */
insert into comsof.connection_unit (id, conn_module_id,	produkt_id, letzten_datum_mod, cluster)
	select  eq_id , enc_eqp_id ,Null , Null, 1 from comsof.out_closures where layer='Feeder';
with sel as (select conmod.id elem_id, connmodule.uuid from comsof.connection_module connmodule  join comsof.connection_unit conmod on conmod.conn_module_id = connmodule.id)
...
/* The id of the "connection_Element" is
	...
	For splitter is the splice_id of the  "outer_splitter"*/
insert into  comsof.connection_element( id,	conn_unit_id,	produkt_id,	typ, subtyp, installation_ziele,	installation_spalte , label,	 mehrdetail, cluster )
	select eq_id, eq_id, null, 'Active', 'Switch / Plugin', Null, Null, Null, null,1 from comsof.out_closures where layer='Feeder'; /*only for POP*/
with sel as (select elem.id elem_id, unit.uuid from comsof.connection_element elem join comsof.connection_unit unit on unit.id= elem.conn_unit_id)
...
/*Ziehschacht : */
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster)
	select id, geom , 'Schacht', 'Ziehschacht / Schacht an grossen Knotenpunkten', 'Grobplanung',Null , 1  from comsof.out_accessstructures where type='HandHole';
/*Verteilerschacht :*/
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster)
	select id, geom , 'Schacht', 'GF_NVT', 'Grobplanung','Eigenausbau' , 1  from comsof.out_accessstructures where type='HandHole' and eq_id in (select enc_eqp_id from comsof.out_closures) ;
	
/*the id of the "connection_module" is eq_id of the "OUT_ACCESSSTRUCTURE"*/     /*Verteilerschacht :*/
insert into comsof.connection_module(id, typ, produkt_id, knoten_id, netzebene_quelle,netzebene_ziel, eigentum_status, foerdert_status, cluster) 
	select eq_id, 'Muffe', NULL, id, 'Hauptebene','Verteilerebene', 'Eigentum', Null,1 from comsof.out_accessstructures where type = 'HandHole' and eq_id in (select enc_eqp_id from comsof.out_closures);
with sel as( select connmodule.id, knot.uuid  knot_uuid from  comsof.connection_module connmodule join comsof.knoten knot on knot.id=connmodule.knoten_id )
	update comsof.connection_module set knoten_uuid=(select knot_uuid from sel where sel.id=comsof.connection_module .id)  where  comsof.connection_module.knoten_uuid is null;

/*The id of the "connection_unit" is the eq_id of the "out_closure" */     /*Verteilerschacht :*/
insert into comsof.connection_unit (id, conn_module_id,	produkt_id, letzten_datum_mod, cluster)
	select  eq_id , enc_eqp_id ,Null , Null, 1 from comsof.out_closures where layer='Distribution' and enc_eqp_id in (select id from comsof.connection_module where typ='Muffe');
with sel as (select conmod.id elem_id, connmodule.uuid from comsof.connection_module connmodule  join comsof.connection_unit conmod on conmod.conn_module_id = connmodule.id)
	update comsof.connection_unit set conn_module_uuid = (select sel.uuid from sel where sel.elem_id=comsof.connection_unit.id) where conn_module_uuid is null;
...
insert into  comsof.connection_element( id,	conn_unit_id,	produkt_id,	typ, subtyp, installation_ziele,	installation_spalte , label,	 mehrdetail, cluster)
	select splice_id, eq_id, null, 'Passive', 'Splitter', Null, Null, Null, null,1 from comsof.out_splitters where enc_eqp_id in (select id from comsof.connection_unit); 
with sel as (select elem.id elem_id, unit.uuid from comsof.connection_element elem join comsof.connection_unit unit on unit.id= elem.conn_unit_id)
	update comsof.connection_element set conn_unit_uuid=(select uuid from sel where sel.elem_id= comsof.connection_element.id ) where conn_unit_uuid is null;
...
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster)
	select id, geom , 'Kabinet', 'GF_NVT', 'Grobplanung','Eigenausbau' , 1  from comsof.out_accessstructures where type='Cabinet' and virtual='F' and id not in (select id from comsof.knoten) ;

/*the id of the "connection_module" is eq_id of the "OUT_ACCESSSTRUCTURE"*/
insert into comsof.connection_module(id, typ, produkt_id, knoten_id, netzebene_quelle,netzebene_ziel, eigentum_status, foerdert_status, cluster) 
	select eq_id, 'Board', NULL, id, 'Haupt-Ebene','Verteiler-Ebene', 'Eigentum', 'Eigenausbau',1 from comsof.out_accessstructures where  type='Cabinet' and virtual='F';
with sel as( select connmodule.id, knot.uuid  knot_uuid from  comsof.connection_module connmodule join comsof.knoten knot on knot.id=connmodule.knoten_id )
	update comsof.connection_module set knoten_uuid=(select knot_uuid from sel where sel.id=comsof.connection_module .id)  where  comsof.connection_module.knoten_uuid is null;

/*The id of the "connection_unit" is the eq_id of the "out_closure" */
insert into comsof.connection_unit (id, conn_module_id,	produkt_id, letzten_datum_mod, cluster)
	select  eq_id , enc_eqp_id ,Null , Null, 1 from comsof.out_closures where layer='Distribution';	
with sel as (select conmod.id elem_id, connmodule.uuid from comsof.connection_module connmodule  join comsof.connection_unit conmod on conmod.conn_module_id = connmodule.id)
	update comsof.connection_unit set conn_module_uuid = (select sel.uuid from sel where sel.elem_id=comsof.connection_unit.id) where conn_module_uuid is null;
...
/* The id of the "connection_Element" is
	...
	For splitter is the splice_id of the  "outer_splitter"*/
insert into  comsof.connection_element(id,	conn_unit_id,	produkt_id,	typ, subtyp, installation_ziele,	installation_spalte , label,	 mehrdetail, cluster)
	select splice_id, enc_eqp_id , null, 'Passive', 'Splitter', Null, Null, Null, null,1 from comsof.out_splitters; /*only for splitter*/
with sel as (select elem.id elem_id, unit.uuid from comsof.connection_element elem join comsof.connection_unit unit on unit.id= elem.conn_unit_id)
	update comsof.connection_element set conn_unit_uuid=(select uuid from sel where sel.elem_id= comsof.connection_element.id ) where conn_unit_uuid is null;	
...
with sel as (
	...
	,sel3 as (select distinct cable_id, st_linemerge(geom), Null produkt, 'Kabel' typ, 'Haupt-Ebene', connmodule_anfang, connmodule_ende, 'Grobplanung', 'Eigenausbau', 'Eigentum' , 'HP : Home-Pass', leng, 1 from sel2 where leng = max_leng)
...
with sel as (
	...
	,sel2 as (select sel.id, sel.geom, st_length(sel.geom) leng,max(st_length(sel.geom) ) over (partition by sel.cable_id) max_leng, sel.cable_id, sel.connmodule_anfang, comsof.out_closures.enc_eqp_id connmodule_ende  from sel join comsof.out_closures on comsof.out_closures.eq_id=sel.c_cl_eq_id)
	,sel3 as (select distinct cable_id, st_linemerge(geom), Null produkt, 'Kabel' typ, 'Verteiler/Drop', connmodule_anfang, connmodule_ende, 'Grobplanung', 'Eigenausbau', 'Eigentum' , 'HP : Home-Pass', leng, 1 from sel2 where leng = max_leng)
	insert into comsof.kabel (id, geom, produkt_id, typ, netzebene, conn_module_anfang, conn_module_ende, planung_status, foerdert_status,eigentum_status,   homeconnect_status, length, cluster) select * from sel3; 
...
insert into comsof.faser (id, geom, kabel_id, netzebene,buendeln_nr , faser_label ,conn_element_anfang, anf_elem_output_nr, anfang_typ,anfang_label, conn_element_ende, end_elem_input_nr ,ende_typ, ende_label,length, cluster)
	...
	Null ende_typ, Null label_ende, Null length, 1 clust from comsof.temp_faser;
...
insert into comsof.knoten (id, geom , typ , subtyp , planung_status, foerdert_status, cluster)
	select distinct id, geom, 'Lage', 'Lage, zugaenglich mit einem Kugelmarker', 'Grobplanung',Null ,1  from comsof.out_accessstructures where type='AccessStructure' and id not in (select id from comsof.knoten);
...
insert into comsof.rohr (id,		geom,	typ,	produkt_id,	planung_status,	foerdert_status,	netzebene,	homeconnect_status,	anzahl_microducts ,cluster)
	select duct_id , st_linemerge(geom), 'Rohrverband' typ,Null produkt, 'Grobplanung' planung_status , Null foerdert,  'Haupt-Ebene' ebene, 'HP : Home-Pass' hc, capacity, 1 clust from comsof.out_feederduct where capacity>1 and st_length(geom)!=0
	union all
	select duct_id , st_linemerge(geom), 'Einzelrohr' typ,Null produkt,  'Grobplanung' planung_status , Null foerdert,  'Haupt-Ebene' ebene, 'HP : Home-Pass' hc, capacity,1 clust from comsof.out_feederduct where capacity=1 and st_length(geom)!=0
	union all
	select duct_id , st_linemerge(geom), 'Rohrverband' typ,Null produkt,  'Grobplanung' planung_status , Null foerdert,  'Verteiler-Ebene' ebene, 'HP : Home-Pass' hc, capacity,1 clust from comsof.out_distributionduct where capacity>1 and st_length(geom)!=0
	union all
	select duct_id , st_linemerge(geom), 'Einzelrohr' typ,Null produkt, 'Grobplanung' planung_status , Null foerdert,  'Verteiler-Ebene' ebene, 'HP : Home-Pass' hc, capacity,1 clust from comsof.out_distributionduct where capacity=1 and st_length(geom)!=0
	union all
	select duct_id , st_linemerge(geom), 'Rohrverband' typ,Null produkt, 'Grobplanung' planung_status , Null foerdert,  'Verteiler-Ebene' ebene, 'HC : Home-Connect' hc, capacity,1 clust from comsof.out_dropduct where capacity>1 and st_length(geom)!=0
	union all
	select duct_id , st_linemerge(geom), 'Einzelrohr' typ,Null produkt,  'Grobplanung' planung_status , Null foerdert,  'Verteiler/Drop' ebene,Null  hc, capacity,1 clust from comsof.out_dropduct where capacity=1 and st_length(geom)!=0	
...
-- manual changes :(
-- 81 82   145, 146
update comsof.temp_microduct set knot_anf=580, knot_ende=172 , stammt_von=265 where id=185;
update comsof.temp_microduct set stammt_von=185 where id=145;
update comsof.temp_microduct set stammt_von=281 where id=174;
update comsof.temp_microduct set stammt_von=82 where id=245;

-- 305, 307
update comsof.temp_microduct set stammt_von=307 where id=303;

-- 249, 250
update comsof.temp_microduct set stammt_von=250 where id=261;

-- 229, 230
update comsof.temp_microduct set stammt_von=230 where id=226;

-- 221, 22
update comsof.temp_microduct set stammt_von=222 where id=129;

-- 217, 219
update comsof.temp_microduct set stammt_von=219 where id=105;

-- 169, 170
update comsof.temp_microduct set stammt_von=170 where id=294;

-- 153 , 154
update comsof.temp_microduct set stammt_von=154 where id=258;

--  97, 98
update comsof.temp_microduct set stammt_von=98 where id=134;


-- 77, 78 
update comsof.temp_microduct set stammt_von=78 where id=158;


--  69 , 70
update comsof.temp_microduct set stammt_von=70 where id=6;

--  9,10
update comsof.temp_microduct set stammt_von=10 where id=121;
...
with sel1 as ( select * from comsof.out_usedsegments where id in (select id from comsof.out_feederroutes) )
	, sel2 as (select id , st_linemerge(geom), case when crossing='T' then 'Querung' Else 'Laengstrasse' end ,
		...
		0.6, 'Unbefestigt', 'Unbekannt', Null, 'Grobplanung', 'Haupt-Ebene' , 'HP : Home-Pass', 1
...
with sel1 as ( select * from comsof.out_usedsegments where id in (select id from comsof.out_distributionroutes) and id not in (select id from comsof.trasse) )
	, sel2 as (select id , st_linemerge(geom), case when crossing='T' then 'Querung' Else 'Laengstrasse'end ,
		case when crossing='T' then 'Pressbohrung/Bodenverdraengung' Else 'Klassischer Tiefbau (Ausschachtung)' end, Null,
		0.6, 'Unbefestigt', 'Unbekannt', Null, 'Grobplanung', 'Verteiler-Ebene' , Null, 1
		from sel1 where st_length(geom)!=0)
		Insert into comsof.trasse (id , geom, typ ,trassenbauverfahren, sonst_bauverfahren, verlege_tief_m, oberflaeche, widmung, foerdert_status, planung_status,netzebene, homeconnect_status ,cluster) select * from sel2;
...
with sel1 as ( select * from comsof.out_usedsegments where id in (select id from comsof.out_droproutes) and id not in (select id from comsof.trasse) )
	, sel2 as (select id , st_linemerge(geom),'Hausanschlusstrasse',
		...
		0.6, 'Unbefestigt', 'Unbekannt', Null, 'Grobplanung', 'Verteiler/Drop' , Null, 1
		from sel1 where st_length(geom)!=0)
		Insert into comsof.trasse (id , geom, typ ,trassenbauverfahren, sonst_bauverfahren, verlege_tief_m, oberflaeche, widmung, foerdert_status, planung_status,netzebene, homeconnect_status ,cluster) select * from sel2;	
...
with sel1 as (select source_id id from comsof.out_usedsegments where crossing='T' and st_length(geom)!=0
	...
	, sel2 as (select id , geom, 'Lage', 'Lage ohne Kugelmarker', 'Grobplanung', Null, Null,1  from comsof.out_nodes where id in (select id from sel1 where id not in (select id from comsof.knoten)) )
...
insert into comsof.rohr_schutzrohr (schutzrohr_id, rohr_id, cluster)
	select s.id, r.id ,1  from comsof.schutzrohr s join comsof.rohr r on st_contains(r.geom, s.geom) order by s.id, r.id;
...
Insert into comsof.strecke (_uuid,typ,cluster)
	select  uuid, 'Trasse', 1 from comsof.trasse
	union all
	select  uuid, 'Microduct', 1 from comsof.microduct
	union all
	select  uuid, 'Schutzrohr', 1 from comsof.schutzrohr;	
