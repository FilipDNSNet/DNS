...
---- view  -- INSERT    trigger
create or replace RULE rule_prj_test_eichwalde_dv_hausanschluss_insert_1
	as on insert to view_b_bb.dv_abschluss
		where new._trig='master'
			DO ALSO (select 'Inserted');
CREATE OR REPLACE RULE rule_prj_test_eichwalde_dv_hausanschluss_insert_2
	as on insert to view_b_bb.dv_abschluss
		where new._trig='dv'
			Do instead (SELECT pop_error(E'Error \n    It is not possible to insert directly to Dynamic-View "dv_abschluss" !','Instead, Insert into the master table "prj_test_eichwalde.adresse_abschluss".'));

---- view  -- DELETE    trigger
CREATE OR REPLACE RULE rule_prj_test_eichwalde_dv_abschluss_delete
	AS ON DELETE TO view_b_bb.dv_abschluss
		where exists (select from prj_test_eichwalde.adresse_abschluss sc where old._adresse_id=sc.adresse_id and old._abschluss_id=sc.abschlusspunkte_id)
			DO INSTEAD (SELECT pop_error(E'Error \n    It is not possible to delete from Dynamic-View "dv_abschluss" !','DELETE from the master table "prj_test_eichwalde.adresse_abschluss".'));


---- view  -- UPDATE    trigger
create or replace function tr_prj_test_eichwalde_dv_abschluss_before_update() returns trigger as $$
declare
	t boolean;
begin
	select case
	...
	if (t) Then
		...
		if (NEW._vid_ = OLD._vid_ or ((NEW._vid_ is null) and (OLD._vid_ is null)))
			...
			and (NEW._cluster_ = OLD._cluster_ or ((NEW._cluster_ is null) and (OLD._cluster_ is null)))
			then
				update prj_test_eichwalde.abschlusspunkte tar set
				...
				update prj_test_eichwalde.knoten tar set
					bez=new._abschluss_bez
					where tar.id=(select knoten_id from prj_test_eichwalde.abschlusspunkte ab where ab.id=new._abschluss_id);
				update prj_test_eichwalde.knoten tar set
					bez=new.nvt_bez
					where tar.id=(select knoten_id from prj_test_eichwalde.abschlusspunkte ab where ab.id=new._nvt_);
				update prj_test_eichwalde.adresse_abschluss tar set
...
END;
$$ language plpgsql;
drop trigger  if  exists tr_prj_test_eichwalde_dv_abschluss_before_update on view_b_bb.dv_abschluss;
CREATE TRIGGER tr_prj_test_eichwalde_dv_abschluss_before_update
	BEFORE UPDATE ON view_b_bb.dv_abschluss
		FOR EACH ROW
			EXECUTE PROCEDURE tr_prj_test_eichwalde_dv_abschluss_before_update();
			

--select * , dns_get_rohr_verbindung(knoten_id, 'prj_test_eichwalde') val from
--	(select * from 
--		(select knoten_id, homeconnect_status, typ, cluster from prj_test_eichwalde.abschlusspunkte abs limit 1) ab     ,   			
...
create or replace function tr_prj_test_eichwalde_adresseabschluss_insert_on_dvabschluss() returns trigger as $$
begin
	insert into view_b_bb.dv_abschluss ( 
		...
		select * , (select bez from prj_test_eichwalde.knoten kn where kn.id=_nvt_ ) nvt_bez, (Select project_name  from _cluster where _cluster.id=_cluster_) _projekt_, 'master' _trig
		from(
			select 
				...
				, knoten_id _knoten_id_, (select bez from prj_test_eichwalde.knoten kn where kn.id=knoten_id) _abschluss_bez, val[2] _verbindung_, val[1]  _farbe_seq_, (val[3])::uuid _nvt_
			from (
					select * , dns_get_rohr_verbindung(knoten_id, 'prj_test_eichwalde') val from
						(select * from 
							(select knoten_id, homeconnect_status, typ, cluster from prj_test_eichwalde.abschlusspunkte  abs where abs.id=new.abschlusspunkte_id) ab   			
...
create trigger tr_prj_test_eichwalde_adresseabschluss_insert_on_dvabschluss
	AFTER insert on prj_test_eichwalde.adresse_abschluss
		for each row
			execute procedure tr_prj_test_eichwalde_adresseabschluss_insert_on_dvabschluss();

---- master  -- DELETE    trigger
CREATE Or REPLACE FUNCTION tr_prj_test_eichwalde_adresseabschluss_delete_on_dvabschluss() returns trigger as $$
begin
...
create trigger tr_prj_test_eichwalde_adresseabschluss_delete_on_dvabschluss
	after delete on prj_test_eichwalde.adresse_abschluss
		for each row
			execute procedure tr_prj_test_eichwalde_adresseabschluss_delete_on_dvabschluss();


---- master -- UPDATE   trigger
CREATE OR REPLACE RULE rule_prj_test_eichwalde_adresseabschluss_update_on_dvabschluss
	AS ON UPDATE TO prj_test_eichwalde.adresse_abschluss
		DO ALSO (
...
create or replace RULE rule_adressen_update_on_dv_prj_test_eichwalde
	as on update to adressen.adressen
		where (new.bezirk_nr is not null and new.bezirk_nr in (select unnest(bezirk_nr) from _cluster where schema_name='prj_test_eichwalde' )) 
			or 
			(new.gemeinde_schluessel is not null and new.gemeinde_schluessel in (select unnest(gemeindeschluessel) from _cluster where schema_name='prj_test_eichwalde' ))
				do also(
...
create or replace RULE rule_knoten_update_on_dv_prj_test_eichwalde_dvabschluss
	as on update to prj_test_eichwalde.knoten
			do also(
...
create or replace RULE rule_abschlusspunkte_update_on_dv_prj_test_eichwalde_dvabschluss
	as on update to prj_test_eichwalde.abschlusspunkte
			do also(
				update view_b_bb.dv_abschluss  set 
						homeconnect_status = new.homeconnect_status
						, typ= new.typ
...
---- TO DO LATER   create or replace RULE rule_microduct_update_on_dv_prj_test_eichwalde_dvabschluss
---- TO DO LATER   	as on update to prj_test_eichwalde.microduct
---- TO DO LATER   			do also(
