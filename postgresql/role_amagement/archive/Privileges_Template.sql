...
BEGIN
   IF NOT EXISTS (
      ...
      WHERE  rolname in ('gr_<db_name>_editor','gr_<db_name>_viewer' ) ) THEN
		CREATE ROLE IF NOT EXISTS gr_<db_name>_editor WITH NOSUPERUSER NOCREATEDB NOLOGIN NOREPLICATION NOCREATEROLE;
		CREATE ROLE IF NOT EXISTS gr_<db_name>_viewer WITH NOSUPERUSER NOCREATEDB NOLOGIN NOREPLICATION NOCREATEROLE;
   END IF;
END
$do$;
...
BEGIN
	...
	EXECUTE 'GRANT SELECT ON ALL SEQUENCES IN SCHEMA '||schs||' TO gr_<db_name>_editor;';
...
BEGIN
    ...
    LOOP
        RAISE NOTICE 'tag:%   classid:%   obj.object_type:%   obj.schema_name:%,  object_identity:%',
                     ...
                     obj.schema_name,
					 obj.object_identity;
		EXECUTE 'GRANT USAGE ON SEQUENCE '||obj.object_identity||' TO gr_<db_name>_editor;';
